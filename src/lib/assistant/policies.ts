// Nội dung chính sách tĩnh cho tool `get_multicart_policy`. Bám đúng nghiệp vụ đã cài đặt
...
export const ASSISTANT_POLICIES: Record<AssistantPolicyTopic, string> = {
  payment:
    "MultiCart hỗ trợ hai hình thức thanh toán: thanh toán khi nhận hàng (COD) và thanh toán trực tuyến qua VNPay. COD chỉ khả dụng khi chính nhà bán của sản phẩm đó bật hỗ trợ COD — không phải sản phẩm nào cũng có COD, hệ thống sẽ báo rõ khi không hỗ trợ. Với VNPay, đơn được tạo ở trạng thái chờ và chỉ chuyển sang đã thanh toán sau khi VNPay xác nhận giao dịch thành công. Không có ví điện tử nội bộ.",
  shipping:
